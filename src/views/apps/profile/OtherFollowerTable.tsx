...
import { getOtherFollowers } from 'src/lib/api'
import { Button, CircularProgress } from '@mui/material'
import { useRouter } from 'next/router'

type SortType = 'asc' | 'desc' | undefined | null

const OtherFollowerTable = ({ profileId }) => {
      ...
      renderCell: (params: GridRenderCellParams) => {
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <CustomAvatar src={row.follower.avatar} sx={{ mr: 3, width: '1.875rem', height: '1.875rem' }} />
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => (
        <Button
          ...
          variant='outlined'
          color='secondary'
          onClick={() => handleSendMoney(params.row.follower.profileId)}
        >
          Send Money
        </Button>
      )
  ...
  const fetchTableData = useCallback(
    async (sort: SortType, keyword: string, column: string, page: number, size: number) => {
      setLoading(true)
      const res = await getOtherFollowers(profileId, {
        keyword,
        sort,
        column,
        page,
    ...
    <Fragment>
      {loading ? (
        ...
        <DataGrid
          ...
          onRowClick={params => router.push(`/profile/${params.row.follower.profileId}`)}
        />
      )}
    </Fragment>
  )
...
export default OtherFollowerTable
